...
*/
  constructor(obj:any={}){
        ...
        if(obj["users"] != undefined && obj["users"] != null && _.isArray(obj["users"])){
            ...
            this["users"] = obj["users"].map((value)=>{
              if(value._class){
                ...
                return new Index["user_roles"](value) ;
              }
            })
    ...
    public _class:string  = "oidc_Client" ;
        ...
        */
               public "users"?:Interface.Iuser_roles[];
          ...
          var promArr:Array<Promise<boolean>> = [Promise.resolve(true)] ;
              ...
              if(target["users"] != null && target["users"] != undefined ){
                  ...
                  target["users"].forEach((_users , index:number)=>{
                    ...
                    promArr.push( Index["user_roles"].check(_users, isCompleteObj , path+"users.")
                      .catch((err)=>{
                        throw new Error(path+"users index: "+ index +"is not user_roles")
                        

                      }) )
                  if(_users._class != null && _users._class != undefined){
                    promArr.push( Index[_users._class].check(_users, isCompleteObj , path+"users.")
                      .catch((err)=>{
                        throw new Error(path+"users index: "+ index +"is not a " + _users._class )
