...
exports.Entity_application_configuration_ferrand = void 0;
const Index = require("./Index");
const application_configuration_1 = require("./application_configuration");
/**
  la conf ferrand
*/
class Entity_application_configuration_ferrand extends application_configuration_1.Entity_application_configuration {
    static cast(obj = {}, castChildClass = false) {
        if ((!castChildClass) && obj._class && obj._class != 'application_configuration_ferrand' && [].indexOf(obj._class) !== -1) {
    ...
    static check(target, isCompleteObj = true, path = "") {
        ...
        if (target.apiUrl != null && target.apiUrl != undefined) {
            res = Entity_application_configuration_ferrand.checkapiUrl(target.apiUrl, `${path}.apiUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
        if (target.bddUrl != null && target.bddUrl != undefined) {
            res = Entity_application_configuration_ferrand.checkbddUrl(target.bddUrl, `${path}.bddUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
