...
exports.Entity_application_configuration_comptoire_or_admin = void 0;
const Index = require("./Index");
const application_configuration_1 = require("./application_configuration");
/**
  cond des comptoire de l'or admin
*/
class Entity_application_configuration_comptoire_or_admin extends application_configuration_1.Entity_application_configuration {
    static cast(obj = {}, castChildClass = false) {
        if ((!castChildClass) && obj._class && obj._class != 'application_configuration_comptoire_or_admin' && [].indexOf(obj._class) !== -1) {
    ...
    static check(target, isCompleteObj = true, path = "") {
        ...
        if (target.bddApiUrl != null && target.bddApiUrl != undefined) {
            res = Entity_application_configuration_comptoire_or_admin.checkbddApiUrl(target.bddApiUrl, `${path}.bddApiUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
        if (target.serviceUrl != null && target.serviceUrl != undefined) {
            res = Entity_application_configuration_comptoire_or_admin.checkserviceUrl(target.serviceUrl, `${path}.serviceUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
