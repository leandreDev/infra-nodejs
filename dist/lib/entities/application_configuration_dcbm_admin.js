...
exports.Entity_application_configuration_dcbm_admin = void 0;
const Index = require("./Index");
const application_configuration_1 = require("./application_configuration");
/**
  configiration de dcbm admin
*/
class Entity_application_configuration_dcbm_admin extends application_configuration_1.Entity_application_configuration {
    static cast(obj = {}, castChildClass = false) {
        if ((!castChildClass) && obj._class && obj._class != 'application_configuration_dcbm_admin' && [].indexOf(obj._class) !== -1) {
    ...
    static check(target, isCompleteObj = true, path = "") {
        ...
        if (target.bddUrl != null && target.bddUrl != undefined) {
            res = Entity_application_configuration_dcbm_admin.checkbddUrl(target.bddUrl, `${path}.bddUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
        if (target.uploadUrl != null && target.uploadUrl != undefined) {
            res = Entity_application_configuration_dcbm_admin.checkuploadUrl(target.uploadUrl, `${path}.uploadUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
        if (target.apiUrl != null && target.apiUrl != undefined) {
            res = Entity_application_configuration_dcbm_admin.checkapiUrl(target.apiUrl, `${path}.apiUrl`);
            if (res && res.length > 0) {
                err = [...err, ...res];
            }
        }
...
exports.Entity_application_configuration_dcbm_admin = Entity_application_configuration_dcbm_admin;
